...
class Buffer(object):
    """A Buffer provides a way of accessing a sequence of token across lines

    Its constructor take an iterator called "the source", that returns the next
    line of token as a list each time it is queried, or None to indicate the end
    of data.

    The Buffer in effect concatenates the sequence returned from its source and 
    then supplies the item from them one at a time throught its pop() method, 
    calling the source for more sequences of itmes only when needed.

    In addition, Buffer provides a current method to look at the next item to be
    supplied, without sequencing past it.

    The __str__ method prints all tokens read so far, up to the end of the current
    ...
    def pop(self):
        """Remove the next item from self and return it. If self
        has exhausted its source, returns None."""
