...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        env = Frame(procedure.env)
        #add
        print('###procedure:',procedure)
        print('###formals:',procedure.formals)
        print('###args:', args)
...
def apply_primitive(procedure, args, env):
    ...
    while args is not nil:
        first, args = args.first, args.second
...
class Frame:
    """An environment frame binds Scheme symbols to Scheme vlaues."""
    ...
    def lookup(self, symbol):
        """Return the value bound to SYMBOL. Error if SYMBOL is not found."""
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) !=  len(vals):
            raise SchemeError("different number of formal parameter and arguments received")
...
class LambdaProcedure:
    ...
    def __init__(self, formals, body, env):
        ...
        whose body is the sigle Scheme expression BODY, and whose parent 
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if len(body) > 1:
            ...
            body = body.first
        env.define(target, LambdaProcedure(formals, body, env))
        return target
    else:
...
def check_formals(formals):
    ...
    for i in range(len(formals)):
        formal = formals[i]
        if not scheme_symbolp(formal) or formal in formals_list:
